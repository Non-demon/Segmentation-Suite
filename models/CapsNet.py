...
class CapsNet(nn.Module):
    def __init__(self, routing_iterations, n_classes = 10):
        ...
        # (n,256,20,20)
        self.primaryCaps = PrimaryCapsLayer(input_channels = 256, output_caps = 32, output_dim = 8, kernel_size = 9,
                                            stride = 2)  # outputs 6*6
        # (n,1152,8)
        # 1152=((20-8)/2)*((20-8)/2)*32
        # for every single pixel, there are [output_caps] caps with dim = [output_dim]
        self.num_primaryCaps = 32 * 6 * 6   # 1152
        routing_module = AgreementRouting(input_caps = self.num_primaryCaps, output_caps = n_classes,
                                          n_iterations = routing_iterations)
        self.digitCaps = CapsLayer(input_caps = self.num_primaryCaps, input_dim = 8, output_caps = n_classes,
                                   output_dim = 16, routing_module = routing_module)
        # (n,10,16)
        # for each img, there are [output_caps] caps with dim = [output_dim]
